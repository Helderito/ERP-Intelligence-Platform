...
Este é um documento vivo: cresce a cada Sprint fechado. A versão atual cobre os conceitos aplicados nos Sprints 00, 01, 02, 03, 04, 05, 06 e 07.
...
**Como foi aplicado no projeto?** O [Domain Model](../database/Domain-Model.md) define os Bounded Contexts (Identity, Master Data, Inventory, Sales, …). No Sprint 02, o Bounded Context de Identity ganhou os seus primeiros Aggregates reais: `User` e `RefreshToken`, cada um com o seu Aggregate Root, e *Value Objects* (`EmailAddress`, `PasswordHash`) — objetos sem identidade própria, comparados pelo seu valor, não por um Id. No Sprint 03, o mesmo contexto foi expandido com `Role`, `Permission`, `UserRole` e `RolePermission` para suportar autorização baseada em permissões. No Sprint 04, nasceu o primeiro Bounded Context fora de Identity: Master Data, com o Aggregate `Product`, o Value Object `ProductCode` e as entidades de referência `Category` e `UnitOfMeasure`. No Sprint 05, Master Data cresceu com `Customer`, `CustomerCode`, `CustomerContact` e `CustomerAddress`; contactos e moradas vivem dentro do Aggregate `Customer`, não como recursos independentes. No Sprint 06, o mesmo padrão foi aplicado a `Supplier`, `SupplierCode`, `SupplierContact` e `SupplierAddress`, mantendo fornecedores separados de Purchasing e Finance. No Sprint 07, `Warehouse` e o Value Object `WarehouseCode` foram adicionados a Master Data, com `WarehouseType` como referência semeada e sem qualquer lógica de stock no Aggregate. No Sprint 08a, `Category` e `UnitOfMeasure` ganharam um ciclo de vida gerido, e `TaxCode` foi adicionado como referência independente com taxa percentual validada, sem antecipar cálculos fiscais. No Sprint 08b, `Country`, `Currency` e `PaymentTerm` foram modelados como entidades de referência imutáveis e semeadas, fechando o Epic de Master Data sem lhes atribuir um ciclo de vida de negócio artificial.
...
**Como foi aplicado no projeto?** O `AppDbContext` mapeia `User` e `RefreshToken`. Como estas entidades usam Value Objects (`EmailAddress`, `PasswordHash`), a configuração usa `OwnsOne` — uma forma de dizer ao EF Core "este Value Object faz parte da mesma tabela do seu dono, não é uma tabela à parte". A migration `AddIdentityAuthentication` foi gerada no Sprint 02 para criar as tabelas `User` e `RefreshToken`; no Sprint 03, a migration `AddRolesAndPermissions` adicionou `Role`, `Permission`, `UserRole` e `RolePermission`, com seed inicial do catálogo mínimo de permissões. No Sprint 04, a migration `AddProductCatalog` adicionou `Product`, `Category` e `UnitOfMeasure`, semeando `General`, `Unit` e `Kilogram`, e também a permission `catalog.manage`. No Sprint 07, `AddWarehouseManagement` adicionou `Warehouse` e `WarehouseType`, a relação restritiva entre ambos, três tipos iniciais e a permission `warehouses.manage` ligada ao Administrator. No Sprint 08a, `AddManagedReferenceData` alterou aditivamente as tabelas existentes de Category e UnitOfMeasure, preenchendo os registos semeados como ativos sem perder IDs ou referências de Product, e criou TaxCode e a permission `reference.manage`. No Sprint 08b, `AddSharedReferenceData` criou as tabelas singulares Country, Currency e PaymentTerm, índices únicos por código e 45 linhas semeadas com Guids determinísticos. No Sprint 09a, `AddCompanyTenancy` criou as tabelas de empresa e associação, acrescentou `CompanyId` primeiro como anulável às sete tabelas existentes, preencheu os dados com a empresa por omissão e só depois tornou a coluna obrigatória com índices e chaves estrangeiras.
...
## Tenancy com Esquema Partilhado

**O que é?** Um modelo multiempresa em que várias empresas usam as mesmas tabelas, mas cada registo empresarial contém um `CompanyId`. O contexto autenticado determina que subconjunto dos dados pode ser consultado ou alterado.

**Como foi aplicado no projeto?** O Sprint 09a introduziu `Company` e `UserCompany`, colocou o `companyId` no JWT e aplicou filtros globais do EF Core a Customer, Supplier, Product, Category, UnitOfMeasure, TaxCode e Warehouse. Um guard em `SaveChanges` impede escritas com uma empresa diferente da empresa atual. Country, Currency, PaymentTerm, WarehouseType e Identity continuam globais por decisão explícita.

**Porque é que a migração teve três passos?** Adicionar imediatamente uma coluna obrigatória partiria todas as linhas já existentes. A migration adiciona `CompanyId` anulável, faz o backfill para uma empresa por omissão determinística e só depois aplica `NOT NULL`, índices por empresa e chaves estrangeiras. Um teste de upgrade executa exatamente esse percurso sobre PostgreSQL real.

**Erros comuns a evitar:** confiar apenas no filtro de leitura, manter códigos globalmente únicos depois de introduzir empresas, ou adicionar `CompanyId` indiscriminadamente a catálogos nacionais e tabelas de Identity.
...
**Como foi aplicado no projeto?** O Sprint 04 implementou a fundação do Product Catalog dentro de Master Data: `Product` como Aggregate Root, `ProductCode` como Value Object imutável, e `Category`/`UnitOfMeasure` como entidades de referência semeadas por migration. Stock, preço, imagens, variantes e códigos de barras ficaram explicitamente fora do Aggregate para não misturar Product Catalog com Inventory ou Pricing. No Sprint 05, o mesmo Bounded Context passou a incluir `Customer`: o código de cliente é imutável, o delete é lógico, e contactos/moradas são entidades internas geridas pela raiz `Customer`, sem endpoints próprios. No Sprint 06, `Supplier` repetiu a mesma abordagem para fornecedores, deixando contratos, price lists, purchase orders, goods receipt e statements fora do Aggregate. No Sprint 07, `Warehouse` passou a representar apenas o registo organizacional do armazém; tipos são referência semeada e stock, movimentos, transferências, localizações e picking continuam no futuro Inventory. No Sprint 08a, Category e UnitOfMeasure evoluíram para catálogos administráveis e TaxCode foi acrescentado com taxa entre 0 e 100; códigos continuam imutáveis, o delete é lógico e os dropdowns operacionais veem apenas referências ativas. No Sprint 08b, catálogos curados de países, moedas e condições de pagamento foram semeados como dados somente de leitura para consumo futuro por Compras, Vendas e Finanças. Esta entrega completa o EP-003 - Master Data.

**Relação com outros conceitos:** Master Data usa DDD para proteger regras de domínio, EF Core para persistência, API REST para exposição e RBAC para garantir que apenas utilizadores com as permissões específicas (`catalog.manage`, `customers.manage`, `suppliers.manage`, `warehouses.manage` ou `reference.manage`) alteram cada área.
...
**Como foi aplicado no projeto?** O Sprint 04 adicionou `GET /products?page=1&pageSize=20&search=...`, `GET /products/{id}`, `POST /products`, `PUT /products/{id}` e `DELETE /products/{id}`. O delete é uma desativação lógica: `Product.IsActive` passa a `false`, mas o registo continua na base de dados para histórico e referências futuras. No Sprint 05, o mesmo padrão foi aplicado a `Customer`, com `GET /customers`, `GET /customers/{id}`, `POST /customers`, `PUT /customers/{id}` e `DELETE /customers/{id}`; os contactos e moradas viajam no payload do cliente. No Sprint 06, `Supplier` ganhou os mesmos endpoints com uma diferença importante no padrão de listagem: `GET /suppliers` devolve uma lista leve (`id`, `code`, `name`, `isActive`), e o detalhe completo com contactos/moradas só é carregado por `GET /suppliers/{id}`. No Sprint 07, Warehouse segue a lista leve e carrega o nome do tipo apenas no detalhe; `GET /warehouse-types` fornece o catálogo read-only para o formulário.
...
**Como foi aplicado no projeto?** `tests/ERP.UnitTests` cobre regras de domínio (por exemplo, `EmailAddress` rejeita formatos inválidos, `Role` rejeita nomes vazios, códigos de Master Data normalizam valores e permanecem imutáveis após criação). `tests/ERP.IntegrationTests` cobre autenticação, autorização e os módulos de Master Data contra uma instância real de PostgreSQL criada por *Testcontainers*. No Sprint 07, a cobertura inclui criação, detalhe com nome do tipo, pesquisa leve, atualização, desativação, tipos semeados e a distinção entre `401` sem autenticação e `403` sem `warehouses.manage`. No Sprint 08a, os testes validam também a migration aditiva em comportamento: referências de Product sobrevivem à desativação de Category/UnitOfMeasure, listas operacionais escondem itens inativos, TaxCode limita a taxa a 0–100 e writes distinguem 401/403. No Sprint 08b, os testes unitários cobrem códigos ISO e `NetDays`, enquanto a integração confirma seeds, ordenação, autenticação e ausência de write endpoints. No Sprint 09a, um teste de upgrade valida o backfill sobre o esquema 08b e outro cria duas empresas para provar isolamento de leitura/escrita e unicidade de códigos por empresa. Vitest + React Testing Library cobrem também a administração da Empresa; 08b não tem UI por decisão de âmbito.
...
**Como foi aplicado no projeto?** O endpoint `POST /auth/login` devolve um `accessToken` (JWT) válido por um curto período (configurável, `AccessTokenMinutes`). O `GET /auth/me`, protegido com `[Authorize]`, só responde com sucesso se receber um JWT válido no cabeçalho `Authorization: Bearer ...`. Desde o Sprint 09a, o token inclui também o `companyId` obtido da associação `UserCompany`, que alimenta o contexto de empresa dos filtros de dados. A chave usada para assinar os tokens nunca está no `appsettings.json` commitado — a aplicação recusa-se mesmo a arrancar se a chave não estiver configurada (ver Gestão de Secrets).
